...
CREATE OR REPLACE FUNCTION pg_catalog.read_intermediate_result(key text, format citus.copy_format default 'csv')
    RETURNS record
...
CREATE OR REPLACE FUNCTION pg_catalog.create_intermediate_result(key text, query text)
    RETURNS bigint
...
CREATE OR REPLACE FUNCTION pg_catalog.broadcast_intermediate_result(key text, query text)
    RETURNS bigint
    LANGUAGE C STRICT VOLATILE
