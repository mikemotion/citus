...
static RemoteFileDestReceiver * CreateRemoteFileDestReceiver(char *resultKey,
															 EState *executorState,
															 List *initialNodeList,
															 bool writeLocalFile);
static void RemoteFileDestReceiverStartup(DestReceiver *dest, int operation,
										  TupleDesc inputTupleDescriptor);
static StringInfo ConstructCopyResultStatement(const char *resultKey);
...
static char * QueryResultFileName(const char *resultKey);
...
{
	text *resultKeyText = PG_GETARG_TEXT_P(0);
	char *resultKeyString = text_to_cstring(resultKeyText);
	...
	resultDest = CreateRemoteFileDestReceiver(resultKeyString, estate, nodeList,
											  writeLocalFile);
...
CreateRemoteFileDestReceiver(char *resultKey, EState *executorState,
							 List *initialNodeList, bool writeLocalFile)
{
	...
	resultDest->resultKey = resultKey;
							  ...
							  TupleDesc inputTupleDescriptor)
{
	...
	const char *resultKey = resultDest->resultKey;
	...
	if (resultDest->writeLocalFile)
	{
		...
		fileName = QueryResultFileName(resultKey);
	...
	foreach(connectionCell, connectionList)
	{
		...
		copyCommand = ConstructCopyResultStatement(resultKey);
 ...
 */
static StringInfo
ConstructCopyResultStatement(const char *resultKey)
{
...
RemoteFileDestReceiverReceive(TupleTableSlot *slot, DestReceiver *dest)
{
	...
	/* send row to nodes */
	SendCopyDataToAll(copyData, 0, connectionList);
...
/*
 * ReceiveQueryResultViaCopy is called when a COPY "resultkey" FROM
 ...
 */
void
ReceiveQueryResultViaCopy(const char *resultKey)
{
	const char *resultFileName = NULL;

	CreateIntermediateResultsDirectory();

	resultFileName = QueryResultFileName(resultKey);
 ...
 *
 * When in a distributed transaction block, the directory is of the form:
 * <user id>_<coordinator node id>_<transaction number>
 *
 * Outside of a transaction block the directory is of the form:
 * <user id>_<process id>
 *
 * We choose not to error out outside of a distributed transaction block,
 * since that would cause issues if IntermediateResultsDirectory is called from the
 * abort callback. Instead, we allow the user to create process-local
 * intermediate results, which could be a handy replacement for temporary
 * tables on secondaries.
 */
static char *
 ...
 *
 * The file is read from the directory returned by IntermediateResultsDirectory, which
...
read_intermediate_result(PG_FUNCTION_ARGS)
{
	...
	resultFileName = QueryResultFileName(resultKeyString);
	statOK = stat(resultFileName, &fileStat);
	if (statOK != 0)
	{
		ereport(ERROR, (errcode_for_file_access(),
						errmsg("result \"%s\" does not exist", resultKeyString)));
	}
