...
-- should not create the index in workers
BEGIN;
SET citus.enable_ddl_propagation to false;
CREATE INDEX not_propagated_index ON dustbunnies(id);
COMMIT;
SELECT run_command_on_workers($$SELECT count(*) FROM pg_indexes where tablename LIKE 'dustbunnies_%'$$);

-- should create the index in workers
SET citus.enable_ddl_propagation to ON;
CREATE INDEX propagated_index ON dustbunnies(id);
SELECT run_command_on_workers($$SELECT count(*) FROM pg_indexes where tablename LIKE 'dustbunnies_%'$$);

-- should not drop the index in workers
SET citus.enable_ddl_propagation to OFF;
DROP INDEX propagated_index;
SELECT run_command_on_workers($$SELECT count(*) FROM pg_indexes where tablename LIKE 'dustbunnies_%'$$);

-- should not truncate the table in workers
TRUNCATE dustbunnies;
SELECT * FROM dustbunnies;

-- should truncate the table in workers
SET citus.enable_ddl_propagation to ON;
TRUNCATE dustbunnies;
SELECT * FROM dustbunnies;

--should create/drop the index in workers
CREATE INDEX propagated_index_2 ON dustbunnies(id);
SELECT run_command_on_workers($$SELECT count(*) FROM pg_indexes where tablename LIKE 'dustbunnies_%'$$);
DROP INDEX propagated_index_2;
SELECT run_command_on_workers($$SELECT count(*) FROM pg_indexes where tablename LIKE 'dustbunnies_%'$$);
